...
{
    class WebServiceAuthorizeUser : AuthorizeAttribute
    {
        private readonly Encoding encoding = Encoding.GetEncoding("UTF-8");
        ...
        private bool Authorize(HttpActionContext actionContext)
        {
            try
            {
                ...
                return credentials[0] == "apikey" && credentials[1] == "herebeyourvoodlemagickey";
            }
            catch (Exception ex)
            {
                Logger.Log(ex);
        ...
        /// <returns>A decoded raw string value in format "apikey:value".</returns>
